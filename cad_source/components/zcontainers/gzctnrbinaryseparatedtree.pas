...
var
  ...
  function GetOptimalTestNode2: Boolean;
  begin
...
begin
  ...
  begin
    TEntsManipulator.CreateSeparator(BoundingBox,TestNode_,PFirstStageData,i);
    ...
    begin
      ...
      repeat
         pobj:=TEntsManipulator.IterateResult2PEntity(pobj);
         ep:=TEntsManipulator.GetBBPosition(TestNode_.plane,TEntsManipulator.GetEntityBoundingBox(pobj^));
         case ep of
  ...
  // Тут как-нибудь бы без копирования передать данные, просто указатель внутренний переприсвоить?
  nul.clear;
