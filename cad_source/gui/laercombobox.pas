...
type
  ...
  TZCADLaerComboBox=class(TWinControl)                                        // Компонент TZCADLaerComboBox
  private
    MyItems:TLayerArray;
    M1:boolean; // Маркер
    CP1:TControlPole;
...
begin
  ...
  begin
    ...
    begin
      ...
      Brush.Style:=bsClear;
      if {Index=-1}not TZCADLaerComboBox(parent).fGetLayerProp(nil,lp) then
      begin
...
procedure TZCADLaerComboBox.ItemsClear;                                       // Очистить список
  var
    n:integer;
begin
...
procedure TZCADLaerComboBox.AddItem(OnOff,Freze,Lock:boolean;ItemName:utf8string;lo:pointer); // Создание пункта в списке
begin
...
procedure TZCADLaerComboBox.DelItem(ItemIndex:integer);                       // Удаление пункта из списка
  var
...
procedure TZCADLaerComboBox.SetItemIndex(AValue:integer);                     // Задаёт значение выбранного элемента списка
begin
...
procedure TZCADLaerComboBox.SetListHeight(AValue:integer);                    // Изменение свойства высоты разворачиваемого списка
begin
  sListHeight:=AValue;
  if sListHeight<30 then sListHeight:=30;
end;

function TZCADLaerComboBox.ReadItemsCount:integer;                            // Возвращает длину списка
begin
  result:=length(MyItems);
end;

function TZCADLaerComboBox.ReadHeight:integer;                                // Возвращает значение параметра высоты
begin
  result:=inherited Height;
end;

function TZCADLaerComboBox.ReadWidth:integer;                                 // Возвращает значение параметра ширины
begin
  result:=inherited Width;
end;

procedure TZCADLaerComboBox.SetHeight(AValue:integer);                        // Задаёт значение параметра высоты
begin
...
procedure TZCADLaerComboBox.SetWidth(AValue:integer);                         // Задаёт значение параметра ширины
begin
...
function TZCADLaerComboBox.ReadFont:TFont;                                    // Возвращает параметр шрифта
begin
  result:=CP1.Font;
end;

procedure TZCADLaerComboBox.SetFont(AValue:TFont);                            // Задаёт значение параметра шрифта
begin
  CP1.Font:=AValue;
  CP1.Invalidate;
end;

function TZCADLaerComboBox.ReadItem(iIndex:integer):TLayerPropRecord;    // Возвращает пункт списка по индексу
begin
  result:=MyItems[iIndex];
end;

procedure TZCADLaerComboBox.UpdateIcon;                                       // Обновляет значки
begin
...
procedure TZCADLaerComboBox.PLDeActivate(Sender:TObject);                     // Закрытие списка
  var
    S,P:TPoint;
begin
...
procedure TZCADLaerComboBox.PLDeActivate2(Data:PtrInt);                       // Закрытие списка 2
begin
  PLDeActivate(nil);
end;

procedure TZCADLaerComboBox.PLDeActivate3(Data:PtrInt);                       // Закрытие списка 3
begin
  if PoleLista<>nil then
  begin
    PoleLista.Free;
...
procedure TZCADLaerComboBox.ObnovitSpisok;                                    // Заполнение (обновление) списка развёрнутого листа
  var
    n:integer;
...
procedure TZCADLaerComboBox.LVKlac(Sender:TObject);                           // Обработков кликов на развёрнутом списке
  var
    LVItem:TListItem;
...
procedure TZCADLaerComboBox.KeyDown(Sender:TObject;var Key:Word;Shift:TShiftState);// Отлавливаем эскей
begin
  if Key=27 then Application.QueueAsyncCall(@PLDeActivate3,0);
end;

function TZCADLaerComboBox.GetOnChange:TNotifyEvent;
begin
  Result:=sOnChange;
end;

procedure TZCADLaerComboBox.SetOnChange(AValue:TNotifyEvent);
begin
  sOnChange:=AValue;
end;

procedure TZCADLaerComboBox.ListChanged;
begin
  if Assigned(sOnChange) then sOnChange(Self);
end;

destructor TZCADLaerComboBox.Destroy;                                         // Уничтожение объекта класса
begin
...
procedure Register;
begin
  RegisterComponents('Misc',[TZCADLaerComboBox]);
end;
