...
function BlocksInBasePreViewExport_com(const Context:TZCADCommandContext;
  ...
  if pb<>nil then
    repeat
      if MatchesMaskList(pb^.Name,BlockNameIncludeMask,';',False) then
        if (BlockNameExcludeMask='')or
          (not MatchesMaskList(pb^.Name,BlockNameExcludeMask,';',False)) then begin
