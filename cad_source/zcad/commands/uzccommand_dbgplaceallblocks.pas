...
function dbgPlaceAllBlocks_com(const Context:TZCADCommandContext;
...
begin
  ...
  if pb<>nil then
    repeat
      ...
      BLINSERT:=Pointer(drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray.CreateObj(
        GDBBlockInsertID{,drawings.GetCurrentROOT}));
      PGDBObjBlockInsert(BLINSERT)^.initnul;
      //(@drawings.GetCurrentDWG^.ObjRoot,drawings.LayerTable.GetSystemLayer,0);
      PGDBObjBlockInsert(BLINSERT)^.init(drawings.GetCurrentROOT,
