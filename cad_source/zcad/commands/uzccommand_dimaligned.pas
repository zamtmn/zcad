...
function DrawAlignedDim_com(const Context:TZCADCommandContext;
  operands:TCommandOperands):TCommandResult;
var
  pd:PGDBObjAlignedDimension;
  // указатель на создаваемый размерный примитив
...
begin
  ...
  if commandmanager.get3dpoint(rscmSpecifyFirstPoint,p1)=IRNormal then begin
    ...
    if commandmanager.Get3DPointInteractive(rscmSpecifySecondPoint,
      p2,@InteractiveLineEndManipulator,pline)=IRNormal then begin
      ...
      pd:=Pointer(drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray.CreateInitObj(
        GDBAlignedDimensionID,drawings.GetCurrentROOT));
      ...
      then begin //if all 3 points were obtained - build primitive in the list of primitives
        ...
        //инициализируем примитив, указываем его владельца
        ...
        //assign the obtained point to the appropriate location primitive
        //присваиваем полученые точки в соответствующие места примитиву
        InteractiveADimManipulator(pd,p3,False);
        //use the interactive function for final configuration entity
        //используем интерактивную функцию для окончательной настройки примитива
        ...
        //Add entity to drawing considering tying to undo-redo
