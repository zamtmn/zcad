...
function ExampleConstructToModalSpace_com(const Context:TZCADCommandContext;
  ...
  begin
    ...
    Result^.VertexArrayInOCS.PushBackData(TzePoint3d.Make(pt2.x,pt1.y,0));
    Result^.VertexArrayInOCS.PushBackData(pt2);
    Result^.VertexArrayInOCS.PushBackData(TzePoint3d.Make(pt1.x,pt2.y,0));
    zcAddEntToCurrentDrawingConstructRoot(Result);
    //добавляем в конструкторскую область
...
begin
  ...
  {pp:=}drawPolyline(TzePoint3d.Make(-20,+5,0),
    TzePoint3d.Make(+20,-5,0),3);
