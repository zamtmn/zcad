...
begin
  ...
  if pum<>nil then begin
    pdbu:=pum^.findunit(SupportPath,InterfaceTranslate,DrawingDeviceBaseUnitName);//получаем модуль БД чертежа
    if pdbu<>nil then
...
initialization
  ...
  CreateCommandFastObjectPlugin(@explDbCheck_com,CommandName,  CADWG,0);
finalization
