...
function explDbCheck_com(const Context:TZCADCommandContext;
...
begin
  pum:=drawings.GetCurrentDWG^.GetDWGUnits;
  //получаем модули подключеные к чертежу
  if pum<>nil then begin
    ...
    if pdbu<>nil then
      while commandmanager.getentity(rscmSelectDestinationEntity,pEntity)=GRNormal do begin
        ...
        if EntVarExt<>nil then begin
          ...
          //ищем у примитива переменную в которой хранится
          //имя связанной переменной из модуля БД чертежа
          if pvn<>nil then begin
            pvnDB:=pdbu.FindVariable(pvn.Data.PTD.GetValueAsString(
              pvn.Data.Addr.Instance));
            if pvnDB<>nil then begin
              ...
              if IsIt(TypeOf(PBaseObject^),typeof(DeviceDbBaseObject)) then begin
                ...
                zcUI.TextMessage(
                  format('DeviceDbBaseObject.NameShortTemplate=%s',
              ...
              if IsIt(TypeOf(PBaseObject^),typeof(CableDeviceBaseObject)) then begin
                ...
                zcUI.TextMessage(
                  format('CableDeviceBaseObject.NumberOfCores=%g',
                  [PCableDeviceBaseObject(PBaseObject)^.NumberOfCores]),TMWOHistoryOut);
              end;
            end;
          end;
        end;
