...
var
  ExportTextToCSVParam:TExportTextToCSVParam;
...
function ExportTextToCSV_com(const Context:TZCADCommandContext;
  ...
  try
        ...
        if (pv^.Selected)and isTextEnt(pv^.GetObjType) then begin
          ...
          if uppercase(pv^.Content)='START' then
            if pstart=nil then
              ...
              ProgramLog.LogOutStr('Other "Start" text marker found',
                LM_Error,LogModeDefault,MO_SM or MO_SH);
        end;
        pv:=drawings.GetCurrentROOT^.ObjArray.iterate(ir);
      until pv=nil;

    if pstart=nil then
      ProgramLog.LogOutStr('"Start" text marker not found',LM_Error,
        LogModeDefault,MO_SM or MO_SH)
    else begin
    ...
    repeat
      ...
      if TryStrToFloat(ts,td) then begin
        ...
        ProgramLog.LogOutFormatStr('"%s" Not a float',[ts],LM_Error,
          LogModeDefault,MO_SM or MO_SH);
    until s='';
    ...
    if Count>0 then begin
      ...
      if pv<>nil then
        repeat
          if (pv^.Selected)and isTextEnt(pv^.GetObjType) then begin
            x:=Getcolumn(pv^.P_insert_in_WCS.x-minx,Widths);
            //Floor((pv^.P_insert_in_WCS.x-minx)/ExportTextToCSVParam.W);
...
initialization
  ...
  if SysUnit<>nil then begin
    SysUnit^.RegisterType(TypeInfo(TExportTextToCSVParam));
    //регистрируем тип данных в зкадном RTTI
    SysUnit^.SetTypeDesk(TypeInfo(TExportTextToCSVParam),
      ['Widths','W','H','FileName'],[FNProgram]);
