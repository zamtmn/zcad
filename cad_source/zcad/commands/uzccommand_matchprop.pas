         ...
         begin
              ...
              if MatchPropParam.ProcessLayer then
                if pd^.vp.Layer<>ps^.vp.Layer then
                  begin
                    ...
                    with TGDBPoinerChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,pd^.vp.Layer) do
                    begin
              ...
              if MatchPropParam.ProcessLineType then
                if pd^.vp.LineType<>ps^.vp.LineType then
                  begin
                    zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                    with TGDBPoinerChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,pd^.vp.LineType) do
                    begin
              ...
              if MatchPropParam.ProcessLineWeight then
                if pd^.vp.LineWeight<>ps^.vp.LineWeight then
                  begin
                    zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                    with TGDBTGDBLineWeightChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,pd^.vp.LineWeight) do
                    begin
              ...
              if MatchPropParam.ProcessColor then
                if pd^.vp.color<>ps^.vp.Color then
                  begin
                    zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                    with TGDBTGDBPaletteColorChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,pd^.vp.color) do
                    begin
              ...
              if MatchPropParam.ProcessLineTypeScale then
                if pd^.vp.LineTypeScale<>ps^.vp.LineTypeScale then
                  begin
                    zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                    with TDoubleChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,pd^.vp.LineTypeScale) do
                    begin
              ...
              begin
                if MatchPropParam.TextParams.ProcessTextStyle then
                  if PGDBObjText(pd)^.TXTStyleIndex<>PGDBObjText(ps)^.TXTStyleIndex then
                    begin
                      zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                      with TGDBPoinerChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,PGDBObjText(pd)^.TXTStyleIndex) do
                      begin
                ...
                if MatchPropParam.TextParams.ProcessTextSize then
                  if PGDBObjText(pd)^.textprop.size<>PGDBObjText(ps)^.textprop.size then
                    begin
                      zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                      with TDoubleChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,PGDBObjText(pd)^.textprop.size) do
                      begin
                ...
                if MatchPropParam.TextParams.ProcessTextOblique then
                  if PGDBObjText(pd)^.textprop.Oblique<>PGDBObjText(ps)^.textprop.Oblique then
                    begin
                      zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                      with TDoubleChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,PGDBObjText(pd)^.textprop.Oblique) do
                      begin
                ...
                if MatchPropParam.TextParams.ProcessTextWFactor then
                  if PGDBObjText(pd)^.textprop.wfactor<>PGDBObjText(ps)^.textprop.wfactor then
                    begin
                      zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                      with TDoubleChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,PGDBObjText(pd)^.textprop.wfactor) do
                      begin
                ...
                if MatchPropParam.TextParams.ProcessTextJustify then
                  if PGDBObjText(pd)^.textprop.justify<>PGDBObjText(ps)^.textprop.justify then
                    begin
                      zcPlaceUndoStartMarkerIfNeed(UndoStartMarkerPlaced,CommandName);
                      with TGDBTTextJustifyChangeCommand.CreateAndPushIfNeed(drawing.UndoStack,PGDBObjText(pd)^.textprop.justify) do
                      begin
