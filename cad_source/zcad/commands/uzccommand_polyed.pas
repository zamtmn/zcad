...
function _3DPolyEd_com_CommandStart(const Context:TZCADCommandContext;
...
begin
  ...
  if pobj<>nil then
    repeat
      begin
        if pobj^.selected  and
          ((pobj^.GetObjType=GDBPolylineID)  or
          (pobj^.GetObjType=GDBCableID)) then begin
          p3dpl:=pointer(pobj);
          system.Break;
        end;
      end;
  ...
  if p3dpl=nil then begin
    ...
    zcUI.Do_PrepareObject(
      nil,drawings.GetUnitsFormat,SysUnit^.TypeName2PTD('TPolyEdit'),@PEProp,
      drawings.GetCurrentDWG);
    drawings.GetCurrentDWG^.wa.SetMouseMode(
      (MGet3DPoint) or (MMoveCamera) or (MRotateCamera));
...
begin
  ...
  if (pc<>nil)or(PCreatedGDBLine<>nil) then begin
  ...
  Result:={mclick}0;
  if not PEProp.setpoint then begin
    ...
    if ptv<>nil then
      repeat
        ...
        if ptvprev<>nil then begin
          l:=sqr(distance2piece(wc,ptvprev^,ptv^));
          if l<PEProp.ldist then begin
            PEProp.ldist:=l;
  ...
  if (PEProp.Action=TSPE_Remove) then begin
    if PEProp.nearestvertex>-1 then begin

      pc:=
        PGDBObjCircle(ENTF_CreateCircle(@drawings.GetCurrentDWG^.ConstructObjRoot,@drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray,
        drawings.
  ...
  if (PEProp.Action=TSPE_Insert) then begin
    if abs(PEProp.vdist-PEProp.ldist)>sqreps then
    begin
      PCreatedGDBLine:=
        PGDBObjLine(ENTF_CreateLine(@drawings.GetCurrentDWG^.ConstructObjRoot,@drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray,
        drawings.
        GetCurrentDWG^.GetCurrentLayer,drawings.GetCurrentDWG^.GetCurrentLType,
        LnWtByLayer,ClByLayer,
        PEProp.
        lvertex1,wc));
      ...
      if PEProp.nearestvertex=0 then begin
        PCreatedGDBLine:=
          PGDBObjLine(ENTF_CreateLine(@drawings.GetCurrentDWG^.ConstructObjRoot,@drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray,
          drawings.
          GetCurrentDWG^.GetCurrentLayer,drawings.GetCurrentDWG^.GetCurrentLType,
        ...
        PCreatedGDBLine^.Formatentity(
          drawings.GetCurrentDWG^,dc);
        PEProp.nearestline:=
          PEProp.nearestvertex;
        PEProp.dir:=-1;
      end
      else if PEProp.nearestvertex=p3dpl^.vertexarrayinwcs.Count-1 then begin
        PCreatedGDBLine:=
          PGDBObjLine(ENTF_CreateLine(@drawings.GetCurrentDWG^.ConstructObjRoot,@drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray,
          drawings.
          GetCurrentDWG^.GetCurrentLayer,drawings.GetCurrentDWG^.GetCurrentLType,
  ...
  if (PEProp.Action=TSPE_Scissor) then begin
    if PEProp.vdist>PEProp.ldist+bigeps then begin
      _tv:=
        NearestPointOnSegment(wc,PEProp.lvertex1,PEProp.lvertex2);
      pc:=
        PGDBObjCircle(ENTF_CreateCircle(@drawings.GetCurrentDWG^.ConstructObjRoot,@drawings.GetCurrentDWG^.ConstructObjRoot.ObjArray,
        drawings.
        GetCurrentDWG^.GetCurrentLayer,drawings.GetCurrentDWG^.GetCurrentLType,
  ...
  if (button and MZW_LBUTTON)<>0 then begin
    if (PEProp.Action=TSPE_Remove)and(PEProp.nearestvertex<>-1) then begin
      if p3dpl^.vertexarrayinocs.Count>2 then
      begin
        polydata.index:=PEProp.nearestvertex;
        if PEProp.nearestvertex=
          p3dpl^.vertexarrayinocs.GetCount then
        ...
        domethod:=
          tmethod(@p3dpl^.DeleteVertex);
                                                  {tmethod(domethod).Code:=pointer(p3dpl.DeleteVertex);
                                                  tmethod(domethod).Data:=p3dpl;}
        undomethod:=
          tmethod(@p3dpl^.InsertVertex);
                                                  {tmethod(undomethod).Code:=pointer(p3dpl.InsertVertex);
                                                  tmethod(undomethod).Data:=p3dpl;}
        with specialize
          GUCmdChgMethods<TPolyData>.CreateAndPush(polydata,domethod,undomethod,
            PTZCADDrawing(drawings.GetCurrentDWG)^.UndoStack,@drawings.AfterAutoProcessGDB) do begin
          comit;
        end;
        ...
        drawings.GetCurrentROOT^.
          FormatAfterEdit(drawings.GetCurrentDWG^,dc);
        //p3dpl^.Format;
        zcRedrawCurrentDrawing;
      end else
        zcUI.TextMessage(
          rscm2VNotRemove,TMWOHistoryOut);
    end;
    if (PEProp.Action=TSPE_Insert)and(PEProp.nearestline<>-1)and(PEProp.dir<>0) then
    begin
      if (PEProp.setpoint)or
        (PEProp.Mode=TPEM_Nearest) then begin
        polydata.{nearestvertex}
          index:=PEProp.nearestline;
        if
        PEProp.dir=1 then
          Inc(
            polydata.{nearestvertex}index);
        ...
        undomethod:=tmethod(@p3dpl^.DeleteVertex);
                                                                         ...
                                                                         tmethod(undomethod).Data:=p3dpl;}
        with
          specialize GUCmdChgMethods<TPolyData>.CreateAndPush(
            polydata,domethod,undomethod,PTZCADDrawing(drawings.GetCurrentDWG)^.UndoStack,@drawings.AfterAutoProcessGDB) do begin
          comit;
        end;

        //p3dpl^.vertexarrayinocs.InsertElement(PEProp.nearestline,PEProp.dir,@wc);
        p3dpl^.
          YouChanged(drawings.GetCurrentDWG^);
    ...
    if (PEProp.Action=TSPE_Scissor) then begin
      if PEProp.vdist>PEProp.ldist+bigeps then begin
        p3dpl2:=
          pointer(p3dpl^.Clone(p3dpl^.bp.ListPos.Owner));
        drawings.GetCurrentROOT^.AddObjectToObjArray(@p3dpl2);
        _tv:=
          NearestPointOnSegment(wc,PEProp.lvertex1,PEProp.lvertex2);
        for i:=
          0 to p3dpl^.VertexArrayInOCS.Count-1 do begin
          if i<PEProp.nearestline then
            p3dpl2^.
              VertexArrayInOCS.DeleteElement(0);
        ...
        p3dpl2^.
          VertexArrayInOCS.InsertElement(0,{-1,}_tv);
        p3dpl^.VertexArrayInOCS.InsertElement(
          p3dpl^.VertexArrayInOCS.Count,{1,}_tv);
        p3dpl2^.Formatentity(drawings.GetCurrentDWG^,dc);
        p3dpl^.Formatentity(drawings.GetCurrentDWG^,dc);
        drawings.GetCurrentROOT^.
          ObjArray.ObjTree.CorrectNodeBoundingBox(p3dpl2^);
      end else begin
        if (PEProp.nearestvertex=0)or
          (PEProp.nearestvertex=p3dpl^.VertexArrayInOCS.Count-1) then begin
          zcUI.TextMessage(
            rscmNotCutHere,TMWOShowError);
          exit;
        end;
        p3dpl2:=
          pointer(p3dpl^.Clone(p3dpl^.bp.ListPos.Owner));
        ...
        for i:=
          ...
          if i>PEProp.nearestvertex then
            p3dpl^.
              VertexArrayInOCS.DeleteElement(PEProp.nearestvertex+1);
...
initialization
  ...
  if SysUnit<>nil then begin
    SysUnit^.RegisterType(TypeInfo(TPolyEdit));
    //регистрируем тип данных в зкадном RTTI
    SysUnit^.SetTypeDesk(TypeInfo(TPolyEdit),['Action','Mode','vdist',
      'ldist','nearestvertex','nearestline','dir','setpoint','vvertex','lvertex1','lvertex2']);
    //Даем програмные имена параметрам, по идее это должно быть в ртти, но ненашел
    SysUnit^.SetTypeDesk(TypeInfo(TSubPolyEdit),
      ['TSPE_Insert','TSPE_Remove','TSPE_Scissor']);
    //Даем человечьи имена параметрам
    SysUnit^.SetTypeDesk(TypeInfo(TPolyEditMode),['TPEM_Nearest','TPEM_Select']);
    //Даем человечьи имена параметрам
  end;
  CreateCommandRTEdObjectPlugin(@_3DPolyEd_com_CommandStart,nil,nil,
    nil,@_3DPolyEd_com_BeforeClick,@_3DPolyEd_com_BeforeClick,nil,nil,'PolyEd',0,0);
