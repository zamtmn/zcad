...
function RotateEnts_com(const Context:TZCADCommandContext;
      ...
      repeat
        if pv^.Selected then begin
          if IsObjectIt(typeof(pv^),typeof(GDBObjWithLocalCS)) then
            ...
            pc:=pv^.vp.BoundingBox.pMin.LerpTo(pv^.vp.BoundingBox.pMax,0.5);
          dispmatr:=uzegeometry.CreateTranslationMatrix(-pc.asVector);
          ...
          with PushCreateTGMultiObjectChangeCommand(@PTZCADDrawing(drawings.GetCurrentDWG)^.UndoStack,dispmatr,im,1) do begin
