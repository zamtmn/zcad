...
type
  ...
  TRotateX_com=object(TRotateZ_com)
    function CreateRotmatr(AAngle:double):TzeTypedMatrix4d;virtual;
  end;

  TRotateY_com=object(TRotateZ_com)
    function CreateRotmatr(AAngle:double):TzeTypedMatrix4d;virtual;
  end;

var
  RotateZ_com:TRotateZ_com;
  RotateX_com:TRotateX_com;
  RotateY_com:TRotateY_com;

implementation

procedure TRotateZ_com.CommandContinue(const Context:TZCADCommandContext);
var
  v1:vardesk;
  td:double;
begin
...
procedure TRotateZ_com.showprompt(mklick:integer);
begin
  case mklick of
    0:inherited;
...
procedure TRotateZ_com.rotate(const rotmatr:TzeTypedMatrix4d;button:byte);
var
...
function TRotateZ_com.CreateRotmatr(AAngle:double):TzeTypedMatrix4d;
begin
  Result:=uzegeometry.CreateRotationMatrixZ(AAngle);
end;

function TRotateZ_com.AfterClick(const Context:TZCADCommandContext;wc:TzePoint3d;
...
initialization
  ...
  RotateZ_com.init('RotateZ',0,0);
  RotateZ_com.NotUseCommandLine:=False;
