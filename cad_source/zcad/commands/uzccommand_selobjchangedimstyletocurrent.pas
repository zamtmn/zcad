...
function SelObjChangeDimStyleToCurrent_com(const Context:TZCADCommandContext;
...
begin
  ...
  if pv<>nil then
    repeat
      if pv^.Selected then
        if (pv^.GetObjType=GDBAlignedDimensionID)or(pv^.GetObjType=GDBRotatedDimensionID)or
          (pv^.GetObjType=GDBDiametricDimensionID) then begin
          pv^.PDimStyle:=prs;
  ...
  if psv<>nil then begin
    repeat
      if psv.objaddr^.Selected then
        if (psv.objaddr^.GetObjType=GDBAlignedDimensionID)or
          (psv.objaddr^.GetObjType=GDBRotatedDimensionID)or
          (psv.objaddr^.GetObjType=GDBDiametricDimensionID) then begin
          PGDBObjDimension(
            psv.objaddr)^.PDimStyle:=prs;
