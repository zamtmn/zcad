...
uses
  ...
  uzeBaseExtender,uzeTypes,uzegeometrytypes,uzeconsts,usimplegenerics;
...
begin
  pEntity.Representation.CreateLine(DC,pEntity^,pEntity.vp,cOneMatrix,p1,p2);
end;
...
begin
  Result:=PGDBObjMText(pEntity)^.ObjMatrix.mtr.v[0].Slice.Normalized.asPoint3d;
end;

function TSmartTextEntExtender.getTextNormal(pEntity:Pointer):TzePoint3d;
begin
  Result:=PGDBObjMText(pEntity)^.ObjMatrix.mtr.v[1].Slice.Normalized.asPoint3d;
end;
    ...
    if FRotateOverride then begin
      if PGDBObjEntity(pEntity)^.bp.ListPos.owner<>nil then begin
        ...
        if PGDBObjEntity(pEntity)^.bp.ListPos.owner<>nil then begin
          ...
          l0:=scalardot(V1.Normalized.asVector,TzeVector3d.CreateRec(cosine,sine,0));
          l0:=arccos(l0);
