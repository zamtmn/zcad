...
interface
uses
  uzegeometrytypes,uzsbVarmanDef,uzctnrvectorstrings,uzccommandsmanager,
  ...
  uzsbLexParser,LCLProc,uzccommandsabstract,LazUtilities,uzeTypes;
const
     commandsuffix='>';
     commandprefix=' ';
type
...
procedure processcommand(var input:string);
function MakeInput(inpt:string):boolean;
begin
...
begin
  try
    ...
    begin
      ...
      if TryStrToFloat(input,len) then begin
        //ввод числа пытаемся трактовать это как расстояние по оси трассировки
        if assigned(drawings.GetCurrentDWG) then
          if assigned(drawings.GetCurrentDWG.wa.getviewcontrol) then begin
            ...
            end else if MakeInput(input) then
              exit
            else begin
              if commandmanager.CurrCmd.pcommandrunning<>nil then begin
          ...
          if commandmanager.FindCommand(uppercase({cmd}command))<>nil then begin
            ...
            if length(cmd)>0 then begin
              ...
              if parsed then begin
                ...
                if IsParsed('_realnumber'#0'_softspace'#0'=,_realnumber'#0'_softspace'#0'=,_realnumber'#0,superexpr,parseresult)then begin
                ...
                end else if IsParsed('_realnumber'#0'_softspace'#0'=,_realnumber'#0,superexpr,parseresult)then begin
                  if drawings.GetCurrentDWG<>nil then
                    if drawings.GetCurrentDWG.wa.getviewcontrol<>nil then begin
                 ...
                 if parseresult<>nil then begin parseresult^.Done;Freemem(Pointer(parseresult));end;
                end else if IsParsed('_realnumber'#0'_softspace'#0,superexpr,parseresult)then begin
                  if drawings.GetCurrentDWG<>nil then
