   ...
   for i:= stNumColumn to edNumColumn-1 do
    begin
      //номер пути
       ...
       result.PushBack(TzePoint3d.Make(ptX,ptY,0));
...
begin
   result:=TVectorOfGDBVertex.Create;
   pt1:=TzePoint3d.Make((stNumColumn*zonaWidthColumn)+(zonaWidthColumn/2)-1,-zonaHeightHead-zonaHeightConnect-zonaHeightDev,0);
   pt2:=TzePoint3d.Make((stNumColumn*zonaWidthColumn)+(zonaWidthColumn/4)-1,-zonaHeightHead-zonaHeightConnect-zonaHeightDev+5,0);
   ...
   result.pushback(TzePoint3d.Make(pt1.x-newPt.x,pt1.y+newPt.y,0));
   pt1:=TzePoint3d.Make(10,10,0);
   pt2:=TzePoint3d.Make(20,20,0);
...
begin
   result:=TVectorOfGDBVertex.Create;
   result.pushback(TzePoint3d.Make((edPtNumColumn*zonaWidthColumn)-1,beforePt.y,0));
   result.pushback(TzePoint3d.Make((edPtNumColumn*zonaWidthColumn),beforePt.y-1,0));
end;
//
begin
   ...
   begin
     ...
     begin
       if lColumnSchemaOneLevel[countColumn].listDevVertex[i].getDevice = pgdbdev then
         ...
         begin
          result:=TzePoint3d.Make(countColumn*zonaWidthColumn,-zonaHeightHead-zonaHeightConnect-zonaHeightDev,0)
//           zonaHeightHead=5;
//zonaHeightConnect=25;
//zonaHeightDev=25;
//zonaWidthColumn=20;
         ...
         begin
           ...
           result:=TzePoint3d.Make(countColumn*zonaWidthColumn,y,0)
         end;
       ...
       begin
          if vertexDev.parent <> nil then
            begin
             ...
             drawStartGroupSchema(TzePoint3d.Make(vertexPoint.x,0,0));
            end
           else
            begin
       ...
       if i <> 0 then
         if vertexDev.Childs[i].getNumGroupConnectDevice <> vertexDev.Childs[i-1].getNumGroupConnectDevice then
           begin
              //zcUI.TextMessage('рисуем стартовую точку - ' + vertexDev.getNMONameDevice,TMWOHistoryOut);
              startNewGroup:=true;
              vertexPoint:=TzePoint3d.Make(vertexPoint.x,0,0);
           end;
