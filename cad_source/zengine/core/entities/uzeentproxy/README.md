...
Подробная диагностика вынесена в отдельный модуль `programlog` `PROXY`.
По умолчанию он выключен и не засоряет обычный лог загрузки. Для
расследования некорректных DXF-файлов включайте его явно:

```text
zcad ... logfile <path> lem PROXY
```

Отключение выполняется обычным модульным переключателем:

```text
zcad ... ldm PROXY
```

Подробное описание принципа работы и правил включения/выключения см. в
`PROXY_LOGGING.md`.
