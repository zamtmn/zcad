...
type
  ...
  GDBObjGenericWithSubordinated=object(GDBObjDrawable)
    ...
    procedure DelSelectedSubitem(
      var drawing:TDrawingDef);virtual;
    procedure AddMi(pobj:PGDBObjSubordinated);
      virtual;abstract;
  ...
  GDBObjSubordinated=object(GDBObjGenericWithSubordinated)
    ...
    procedure createfield;virtual;
    destructor done;virtual;
    procedure postload(var context:TIODXFLoadContext);
      virtual;abstract;
...
procedure GDBObjGenericWithSubordinated.FormatAfterDXFLoad(var drawing:TDrawingDef;
  var DC:TDrawContext);
begin
end;
...
begin
  {$IFNDEF DELPHI}
  if FindInvalidUTF8Codepoint(@s[1],length(s),False)<>-1 then begin
    s:=Tria_AnsiToUtf8(s);
    Result:=True;
  end else
  {$ENDIF}
...
begin
  ...
  vv:=UnicodeStringReplace(vv,'@@[ShortName]','@@[NMO_BaseName]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[Name_Template]','@@[NMO_Template]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[Material]','@@[DB_link]',ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[HeadDevice]','@@[GC_HeadDevice]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[HeadDShortName]','@@[GC_HDShortName]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[GroupInHDevice]','@@[GC_HDGroup]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[NumberInSleif]','@@[GC_NumberInGroup]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[RoundTo]','@@[LENGTH_RoundTo]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[Cable_AddLength]','@@[LENGTH_Add]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[Cable_Scale]','@@[LENGTH_Scale]',
    ReplaceAllIgnoreCase);
  vv:=UnicodeStringReplace(vv,'@@[TotalConnectedDevice]',
    '@@[CABLE_TotalCD]',ReplaceAllIgnoreCase);
...
begin
  ...
  nevname:='';
  tt:=uppercase(vt);
  if vn='Name' then begin
    nevname:='NMO_Name';
  end;
  if vn='ShortName' then begin
    nevname:='NMO_BaseName';
  end;
  if vn='Name_Template' then begin
    nevname:='NMO_Template';
  end;
  if vn='Material' then begin
    nevname:='DB_link';
  end;
  if vn='HeadDevice' then begin
    nevname:='GC_HeadDevice';
  end;
  if vn='HeadDShortName' then begin
    nevname:='GC_HDShortName';
  end;
  if vn='GroupInHDevice' then begin
    nevname:='GC_HDGroup';
  end;
  if vn='NumberInSleif' then begin
    nevname:='GC_NumberInGroup';
  end;
  if vn='RoundTo' then begin
    nevname:='LENGTH_RoundTo';
  end;
  if vn='Cable_AddLength' then begin
    nevname:='LENGTH_Add';
  end;
  if vn='Cable_Scale' then begin
    nevname:='LENGTH_Scale';
  end;
  if vn='TotalConnectedDevice' then begin
    nevname:='CABLE_TotalCD';
  end;
  if vn='Segment' then begin
    nevname:='CABLE_Segment';
  end;
  if vn='Cable_Type' then begin
    nevname:='CABLE_Type';
  end;
  if (vn='GC_HDGroup')  and (vt<>'String') then begin
    vt:='String';
  end;
  if (tt='GDBINTEGER') then begin
    vt:='Integer';
  end;
  if (tt='GDBDOUBLE') then begin
    vt:='Double';
  end;
  if (tt='GDBBOOLEAN') then begin
    vt:='Boolean';
  end;
  if (tt='GDBANSISTRING') then begin
    vt:='AnsiString';
  end;
  if (tt='GDBSTRING') then begin
    vt:='String';
  end;
  OldVersTextReplace(vv);
  if nevname<>'' then begin
    vn:=nevname;
  end;
