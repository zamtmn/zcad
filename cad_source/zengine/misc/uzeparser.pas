...
begin
  if APart.TokenInfo.ProcessorClass=nil then
    //явное приведение к типу - обход https://gitlab.com/freepascal.org/fpc/source/-/issues/39387
    //todo: убрать после закрытия бага
    Raise Exception.CreateFmt(rsProcessorClassNilError,[String(GManipulator.GetHumanReadableAdress(APart.TextInfo.TokenPos.P))]);
  ...
  for i:=0 to Parts.size-1 do begin
    ...
    if (parts[i].TokenInfo.ProcessorClass<>nil)and(not(TTokenOptions.IsAllPresent(parts[i].TokenInfo.Options,TGOFake))) then begin
      ...
      if not TTokenOptions.IsAllPresent(parts[i].TokenInfo.Options,TGOSeparator) then
        //явное приведение к типу - обход https://gitlab.com/freepascal.org/fpc/source/-/issues/39387
        //todo: убрать после закрытия бага
        Raise Exception.CreateFmt(rsRunTimeError,[String(GManipulator.GetHumanReadableAdress(parts[i].TextInfo.TokenPos.P))]);
    end;
  end;
end;
{TTokenData=record
